...
static int
create_directory(CLI *cli, char *path, int parents, int verbose) {
	char dir[PATH_MAX], buf[PATH_MAX];
	char *ptr, *save, *dirs[255];
	FL_DIR dirstat, *dirp;
	int i = 0;

	cli_clean_path(cli, path, dir);
...
int
cmd_mkdir_exec(CLI *cli, char *argv[]) {
	...
	for (i=optind+1; argv[i]; i++) {
		create_directory(cli, argv[i], parents, verbose);
	}
